# This example runs a single OFDM frame without CP and wtih no channel estimation/equalization.
# It shows the unequalized constellation at the RX side, along with the channel phase and amplitude.
# Parameters to adjust: Fs, Fc, delay, N, Non, ip
...
I = 1 - 2 * (np.random.randn(N) > 0)
Q = 1 - 2 * (np.random.randn(N) > 0)
...
system.configureTx(usrpName="usrp1", txStreamingConfig=TxStreamingConfig(samples=MimoSignal(signals=[txSig]),
                                                                         sendTimeOffset=0.0))
